...
const FilterPanel = ({ setFilters }) => {
  ...
  <div className="bg-white p-8 rounded-2xl shadow-md border border-gray-200 mb-10">
    <h3 className="text-2xl font-bold text-gray-800 mb-6 border-b pb-2">🎯 Filters</h3>
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {fields.map((field) => (
        <div key={field} className="flex flex-col">
          <label className="text-sm text-gray-700 font-medium mb-1 capitalize">
            {field.replaceAll("_", " ")}
          </label>
          <select
            name={field}
            onChange={handleChange}
            className="w-full border border-gray-300 bg-gray-50 text-sm rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
